  ...
  module StorageAdapter
    ...
    class SqsAdapter < BaseAdapter
      ...
      def self.create_uri(path:, parts: 1)
        ...
        "sqs://#{DerivativeZoo.config.aws_sqs_queue}.sqs.#{DerivativeZoo.config.aws_sqs_region}.amazonaws.com/#{file_path}"
      end
      ...
      def write
        ...
        raise DerivativeZoo::MaxQqueueSize(batch_size: batch_size) if batch_size > DerivativeZoo.config.aws_sqs_max_batch_size
        batch = []
      ...
      def client
        @client ||= if DerivativeZoo.config.aws_sqs_access_key_id && DerivativeZoo.config.aws_sqs_secret_access_key
                      Aws::SQS::Client.new(
                        region: DerivativeZoo.config.aws_sqs_region,
                        credentials: Aws::Credentials.new(
                          DerivativeZoo.config.aws_sqs_access_key_id,
                          DerivativeZoo.config.aws_sqs_secret_access_key
                        )
                      )
                    else
                      Aws::SQS::Client.new(region: DerivativeZoo.config.aws_sqs_region)
                    end
      end
      ...
      def queue_name
        ...
        raise DerivativeZoo::QueueMissingError
      end
    end
  end
end
